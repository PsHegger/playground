#pragma version(1)
#pragma rs_fp_full
#pragma rs java_package_name(pshegger.github.io.playground.mandelbrotrs)
...
uchar4 RS_KERNEL mandelbrot(uchar4 in, uint32_t x, uint32_t y) {
    ...
    double nzr, nzi;
    double zr = 0, zi = 0;
    double cr = left + (right - left) * (x / (double) width);
    double ci = top + (bottom - top) * (y / (double) height);
